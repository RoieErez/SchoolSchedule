...
{
    public partial class SecretaryReplyMSG : Form
    {
        ...
        public SecretaryReplyMSG(Secretary temp1, string temp2)
        {
            InitializeComponent();
            secretary = temp1;

            SqlConnection con = new SqlConnection(constring);
            String query = "Select * from Students where Name = '" + temp2 + "'";
            SqlDataAdapter sda = new SqlDataAdapter(query, con);
            DataTable dt = new DataTable();
            sda.Fill(dt);

            student = new Student(dt.Rows[0][1].ToString(), dt.Rows[0][0].ToString(), "empty", dt.Rows[0][2].ToString(), dt.Rows[0][3].ToString(), true);
            con.Close();
        }
